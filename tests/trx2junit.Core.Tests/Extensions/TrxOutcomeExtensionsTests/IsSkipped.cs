...
public class IsSkipped
{
    ...
    private static IEnumerable<TestCaseData> Value_given___correct_result_TestCases()
    {
        ...
        yield return new TestCaseData((int)TrxOutcome.NotExecuted) .Returns(true);
        yield return new TestCaseData((int)TrxOutcome.NotRunnable) .Returns(true);
        yield return new TestCaseData((int)TrxOutcome.Disconnected).Returns(true);

        yield return new TestCaseData((int)TrxOutcome.Aborted)            .Returns(false);
        yield return new TestCaseData((int)TrxOutcome.Completed)          .Returns(false);
        yield return new TestCaseData((int)TrxOutcome.Error)              .Returns(false);
        yield return new TestCaseData((int)TrxOutcome.Failed)             .Returns(false);
        ...
        yield return new TestCaseData((int)TrxOutcome.Timeout)            .Returns(false);
