...
public class IsSuccess
{
    ...
    private static IEnumerable<TestCaseData> Value_given___correct_result_TestCases()
    {
        ...
        yield return new TestCaseData((int)TrxOutcome.Completed)          .Returns(true);
        yield return new TestCaseData((int)TrxOutcome.Passed)             .Returns(true);
        yield return new TestCaseData((int)TrxOutcome.PassedButRunAborted).Returns(true);
